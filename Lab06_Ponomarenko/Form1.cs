...
namespace Lab06_Ponomarenko
{
    public partial class Form1 : Form
    {
        ...
        private void button_fileKey_out_Click(object sender, EventArgs e)
        {
            ...
            {
                textBox_fileKey_out.Text = saveFileDialog.FileName; // шлях до файлу, де
                                                                    // буде зберігатись ключ
                try
                {
                    ...
                    comboBox_keyLength_byte.Text =
                    this.randomArray.Length.ToString();
                }
                catch (Exception)
                {
...

{
        ...
        private void checkBox_hand_CheckedChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (pathFileKey == "")
                {
                    MessageBox.Show("Вкажіть шлях до файлу з ключем",
                        "Помилка в textBox_fileKey_out", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    MessageBox.Show("Файл з ключем не існує, створіть файл",
                        "Помилка файлу з ключем", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    checkBox_hand.Checked = true;
                    tabControl1.SelectedIndex = 0;
        ...
        public byte[] stringToByteArray(string textHEX) // stringHEX ->; byte[]
        {
            ...
            {
                bs[i] = (byte)Int32.Parse(textHEX.Substring(i * 2, 2),
        ...
        private void button_OK_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Вкажіть шлях до файлу з ключем",
                    "Помилка в textBox_fileKey", MessageBoxButtons.OK, MessageBoxIcon.Information);
                textBox_fileKey_out.Focus();
                return;
            }
            if (!File.Exists(pathFileKey))
            ...
            {
                this.randomArray =
                KeyGen.generator_Key(int.Parse(comboBox_keyLength_byte.Text));
            }
            if (radioButton_passWord.Checked)
            {
                PassWordGen pass = new PassWordGen(textBox_password.Text,
                comboBox_keyLength_byte.Text);
        ...
        private void button_fileCipher_Click(object sender, EventArgs e)
        {
            ...
            if (lenFile == 0)
            {
                MessageBox.Show("Щось не те з файлом", "Застереження",
            ...
            if (lenKey == 0)
            {
                MessageBox.Show("Щось не те з ключем", "Застереження",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            this.Cursor = Cursors.WaitCursor;
            ...
            {
                this.resultArray[i] = (byte)(this.fileArray[i] ^ this.randomArray[i %
                lenKey]); //або можна використовувати BitArray.Xor
            }
        ...
        private void radioButton_passWord_CheckedChanged(object sender,
        EventArgs e)
        {
            if (radioButton_passWord.Checked)
            {
        ...
        {
            MessageBox.Show("Студент групи ЗБ234стн\r\nКушніренко Ю. М.", "Реквізити автора", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
