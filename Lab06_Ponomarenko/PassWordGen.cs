...
namespace Lab06_Ponomarenko
{
    class PassWordGen
    {
        ...
        const int myIterations = 1000; // поки не використовую (ця кількість ітерацій впливає на ключ,
                                       // який буде сформовано з пароля)
        byte[] salt0 = new byte[] { 0, 0, 0, 0, 0, 0, 0, 0 }; // не менше 8 байт (цей масив впливає на ключ,
                                                              // який буде сформовано з пароля)
        byte[] _result;
        public PassWordGen(string usageText, string usageLen) //конструктор класа
        {
            passWord = usageText; // поле для читання може бути ініціалізувати або
                                  // змінено в конструкторі після компіляції
